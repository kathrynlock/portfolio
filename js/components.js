...
function Nav({ page, setPage }) {
  ...
  return (
    <nav style={{
      ...
      padding: '14px 44px',
      background: scrolled ? 'rgba(253,250,255,0.88)' : 'transparent',
      ...
      <button onClick={() => setPage('home')} style={{
        ...
        letterSpacing: '0.04em', display: 'flex', alignItems: 'center', gap: '12px', padding: 0,
      }}>
        <img src="assets/kl-logo.png" alt="KL" style={{ width: '46px', height: '46px', borderRadius: '50%', objectFit: 'cover', boxShadow: '0 2px 10px rgba(58,47,74,0.18)' }} />
        <span>kate lock</span>
      </button>
...
function Divider() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '0 60px', margin: '8px 0' }}>
...
function ThoughtBubble({ thought, delay, floatAnim, posStyle }) {
  ...
  const shadowColors = ['rgba(138,115,151,0.55)', 'rgba(180,160,100,0.35)', 'rgba(100,155,200,0.4)'];
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: vis ? `${floatAnim} ${3.5 + (delay % 500) * 0.002}s ease-in-out infinite` : 'none',
        cursor: 'pointer',
      }}
    >
      {/* Question face */}
      <div style={{
        ...
        <p style={{ fontFamily: 'Outfit', fontSize: '15px', fontWeight: 600, color: 'var(--text)', lineHeight: 1.55, textAlign: 'center' }}>
          {thought.q}
        </p>
      ...
      <div style={{
        ...
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '19px', color: 'white', lineHeight: 1.5, textAlign: 'center' }}>
          {thought.a}
        </p>
      </div>
    </div>
        ...
        <div style={{
          ...
          {preview.map((p, i) => (
            <div key={p.id} className="lift" onClick={() => setPage('projects')} style={{
              minWidth: `${cardW}px`, height: '280px',
              ...
              {/* photo banner */}
              <div style={{ height: '90px', background: p.color, flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
                <svg width="100%" height="90" style={{ position: 'absolute', inset: 0 }} xmlns="http://www.w3.org/2000/svg">
                  <defs>
                  ...
                  <rect width="100%" height="90" fill={`url(#cpat-${p.id})`} />
                </svg>
              ...
              {/* content */}
              <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flex: 1 }}>
                <div>
                  <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '20px', color: 'var(--text)', lineHeight: 1.2, marginBottom: '6px' }}>{p.title}</div>
                  <div style={{ fontFamily: 'Outfit', fontSize: '11px', color: 'var(--text-mid)', lineHeight: 1.4, marginBottom: '10px' }}>{p.brief}</div>
                </div>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: 'var(--text)',
        borderRadius: '24px',
        boxShadow: '6px 6px 0px rgba(58,47,74,0.25)',
...
function PhotoStack() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
      <div onClick={advance} style={{ position: 'relative', width: '300px', height: '340px', cursor: 'pointer' }} title="click to cycle photos">
        {photos.map((photo, i) => {
          const offset = (i - current + photos.length) % photos.length;
          const rotations = [-3, 2, -1.5, 1];
          const rot = rotations[i % rotations.length];
          const zIndex = photos.length - offset;
          const translateY = offset === 0 ? (animating ? -12 : 0) : offset * 5;
          const translateX = offset === 0 ? 0 : offset % 2 === 0 ? -6 : 6;
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{
                background: 'white', borderRadius: '4px', padding: '12px 12px 40px',
                boxShadow: offset === 0
                  ? '8px 8px 24px rgba(58,47,74,0.18), 0 2px 8px rgba(58,47,74,0.08)'
                  : '4px 4px 12px rgba(58,47,74,0.1)',
                height: '100%',
              }}>
                <div style={{
                  background: photo.bg, height: '240px', borderRadius: '2px',
                ...
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '16px', color: 'var(--text-mid)', textAlign: 'center', marginTop: '10px' }}>
                  {photo.note}
                </div>
              </div>
            </div>
...
function TweaksPanel({ tweaks, setTweaks, visible }) {
  ...
  return (
    <div style={{
      ...
      background: 'white', borderRadius: '24px', padding: '28px 26px',
      boxShadow: '0 12px 48px rgba(58,47,74,0.18)', width: '240px',
